import { type Team, type Tournament, type Match, TournamentFormat } from "./types"
...
export function generateBracket(
  ...
  if (format === TournamentFormat.POOL_PLAY) {
    return generatePoolPlayBracket(teams, name, description, createdBy, options)
  }

  // For other formats, respect the user's choice
  switch (format) {
    case TournamentFormat.SINGLE_ELIMINATION:
      return generateSingleEliminationBracket(teams, name, description, createdBy, options)
    case TournamentFormat.DOUBLE_ELIMINATION:
      return generateDoubleEliminationBracket(teams, name, description, createdBy, options)
    case TournamentFormat.ROUND_ROBIN:
      return generateRoundRobinBracket(teams, name, description, createdBy, options)
    default:
      return generateSingleEliminationBracket(teams, name, description, createdBy, options)
  }
}
...
function generateSingleEliminationBracket(
  ...
  const { earlyRoundGames = 1, quarterFinalGames = 3, semiFinalGames = 3, finalGames = 3 } = options
  ...
  for (let i = 0; i < firstRoundMatchCount; i++) {
    ...
    if (seedPosition1 < totalTeams && !assignedTeams.has(sortedTeams[seedPosition1].id)) {
      team1Index = seedPosition1
      assignedTeams.add(sortedTeams[seedPosition1].id)
    }

    // Assign team 2 if available
    if (seedPosition2 < totalTeams && !assignedTeams.has(sortedTeams[seedPosition2].id)) {
  ...
  const byeMatches = matches.filter((match) => match.round === 0 && match.isBye)

  byeMatches.forEach((byeMatch) => {
    const nextMatchPosition = Math.floor(byeMatch.position / 2)

    const nextMatch = matches.find((m) => m.round === 1 && m.position === nextMatchPosition)
...
function generateDoubleEliminationBracket(
  ...
  for (let round = 1; round < totalWinnerRounds; round++) {
    ...
    for (let i = 0; i < matchesInRound; i++) {
      ...
      matches.push({
        ...
        isBye: false,
        isWinnersBracket: true,
        loserGoesTo: round < totalWinnerRounds - 1 ? `l-match-${loserRound}-${loserPosition}` : null,
        bestOf,
      })
    }
  }
  ...
  const byeMatches = matches.filter((match) => match.round === 0 && match.isBye && match.isWinnersBracket)

  byeMatches.forEach((byeMatch) => {
    const nextMatchPosition = Math.floor(byeMatch.position / 2)

    const nextMatch = matches.find((m) => m.round === 1 && m.position === nextMatchPosition && m.isWinnersBracket)
  ...
  for (let round = 1; round < totalLoserRounds; round++) {
    ...
    if (isConsolidationRound) {
      ...
      matchesInRound = matches.filter((m) => !m.isWinnersBracket && m.round === totalWinnerRounds + round - 1).length
    } else {
...
function generatePoolPlayBracket(
  ...
  sortedTeams.forEach((team, index) => {
    ...
    const actualPoolIndex = poolIndex < numPools ? poolIndex : 2 * numPools - 1 - poolIndex
...
function createRoundRobinSchedule(teams: Team[]) {
  ...
  for (let round = 0; round < rounds; round++) {
    ...
    for (let i = 0; i < n / 2; i++) {
      ...
      const team1Id = i === 0 ? teamIds[0] : teamIds[((round + i) % (n - 1)) + 1]
      // For the opponent, rotate the teams
      const team2Id = i === 0 ? teamIds[((round + 1) % (n - 1)) + 1] : teamIds[((round + team2Index) % (n - 1)) + 1]
