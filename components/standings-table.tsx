...
interface StandingsTableProps {
  ...
  onUpdatePositions?: (teamPositions: Array<{ teamId: string; position: number }>, poolId: string) => void
...
export default function StandingsTable({
  ...
  const teamStats = useMemo(() => {
    ...
    matches.forEach((match) => {
      ...
      if (!match.team1Id || !match.team2Id || !stats[match.team1Id] || !stats[match.team2Id]) {
        return
      }
      ...
      if (match.winnerId || (match.team1Score !== undefined && match.team2Score !== undefined)) {
        ...
        if (match.team1Score !== undefined && match.team2Score !== undefined) {
          ...
          if (match.team1Games && match.team2Games) {
            const validGames = match.team1Games.filter(
              (score, i) => score > 0 || (match.team2Games && match.team2Games[i] > 0),
            )
          ...
          if (match.team1TotalPoints !== undefined && match.team2TotalPoints !== undefined) {
  ...
  const displayTeamStats = useMemo(() => {
    ...
    Object.entries(manualPositions).forEach(([teamId, position]) => {
      const teamIndex = statsWithManualPositions.findIndex((stat) => stat.team.id === teamId)
      if (teamIndex !== -1) {
    ...
    if (isEditingPositions) {
      return statsWithManualPositions.sort((a, b) => {
        const posA =
          a.team.id in manualPositions ? manualPositions[a.team.id] : a.manualPosition || Number.MAX_SAFE_INTEGER
        const posB =
          b.team.id in manualPositions ? manualPositions[b.team.id] : b.manualPosition || Number.MAX_SAFE_INTEGER
        return posA - posB
      })
    }
  ...
  // Handle drag start
  const handleDragStart = (e: React.DragEvent<HTMLTableRowElement>, teamId: string) => {
  ...
  // Handle drag over
  const handleDragOver = (e: React.DragEvent<HTMLTableRowElement>, teamId: string) => {
    e.preventDefault()
  ...
  // Handle drag enter
  const handleDragEnter = (e: React.DragEvent<HTMLTableRowElement>, teamId: string) => {
    e.preventDefault()
  ...
  // Handle drop
  const handleDrop = (e: React.DragEvent<HTMLTableRowElement>, targetTeamId: string) => {
    e.preventDefault()
    if (e.currentTarget) {
      e.currentTarget.classList.remove("bg-gray-100", "dark:bg-gray-800")
    }
    ...
    if (sourceTeamId !== targetTeamId) {
      ...
      const sourceIndex = displayTeamStats.findIndex((stat) => stat.team.id === sourceTeamId)
      const targetIndex = displayTeamStats.findIndex((stat) => stat.team.id === targetTeamId)
  ...
  const saveManualPositions = () => {
    ...
    const teamPositions = Object.entries(manualPositions).map(([teamId, position]) => ({
      teamId,
      position,
    }))
  ...
  const formatTeamName = (team: Team, index: number, stats: TeamStats) => {
    ...
    const poolId =
      team.poolId || (isPoolPlay ? tournament.pools?.find((p) => p.teams.some((t) => t.id === team.id))?.id : undefined)
    ...
    const allPoolMatchesCompleted =
      isPoolPlay && poolId !== undefined
        ? tournament.matches
            .filter((m) => m.poolId !== undefined && m.poolId.toString() === poolId.toString())
      ...
      <div>
        <div className="font-medium">
          ...
          {team.players && team.players.length > 0 && (
            <div className="text-xs text-muted-foreground mt-1">{team.players.join(" / ")}</div>
          )}
        </div>
  ...
  return (
    <div className="rounded-md border">
      {currentUser?.isAdmin && poolId && (
        <div className="p-2 border-b flex justify-between items-center">
          {isEditingPositions ? (
            <div className="flex gap-2">
              <Button size="sm" variant="outline" onClick={() => setIsEditingPositions(false)}>
                Cancel
              </Button>
            ...
            <Button size="sm" variant="outline" onClick={() => setIsEditingPositions(true)}>
              Edit Positions
            </Button>
          )}
        </div>
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            {onToggleQualification && currentUser?.isAdmin && <TableHead className="text-center">Qualify</TableHead>}
            {isEditingPositions && currentUser?.isAdmin && <TableHead className="text-center">Move</TableHead>}
          </TableRow>
        </TableHeader>
        <TableBody>
          {displayTeamStats.map((standing, index) => (
            <TableRow
              ...
              className={`
                ...
                ${dragOverTeamId === standing.team.id ? "border-t-2 border-primary" : ""}
                ${isEditingPositions ? "cursor-move" : ""}
              `}
              draggable={isEditingPositions}
              onDragStart={isEditingPositions ? (e) => handleDragStart(e, standing.team.id) : undefined}
              onDragOver={isEditingPositions ? (e) => handleDragOver(e, standing.team.id) : undefined}
              onDragEnter={isEditingPositions ? (e) => handleDragEnter(e, standing.team.id) : undefined}
              onDragLeave={isEditingPositions ? handleDragLeave : undefined}
              onDrop={isEditingPositions ? (e) => handleDrop(e, standing.team.id) : undefined}
              onDragEnd={isEditingPositions ? handleDragEnd : undefined}
            >
              <TableCell className="text-center font-medium">{index + 1}</TableCell>
              <TableCell>
                <div className="flex items-center">
                  {isEditingPositions && <GripVertical className="h-4 w-4 mr-2 text-muted-foreground cursor-grab" />}
              ...
              <TableCell className="text-center">{standing.gamesLost}</TableCell>
              <TableCell className="text-center font-medium">{standing.goalDifference}</TableCell>
              <TableCell className="text-center font-medium">{standing.pointsWon}</TableCell>
              <TableCell className="text-center font-bold">{standing.points}</TableCell>
              {onToggleQualification && currentUser?.isAdmin && poolId && (
              ...
              {isEditingPositions && currentUser?.isAdmin && (
                <TableCell className="text-center">
      ...
      <div className="p-2 text-xs text-muted-foreground">
        <p>
          P = Matches Played, W = Wins, L = Losses, GW = Games Won, GL = Games Lost, PD = Point Difference, TP = Total
          Points Scored, Pts = Points
        </p>
      </div>
    </div>
  )
